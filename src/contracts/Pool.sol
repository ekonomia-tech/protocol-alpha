...
contract Pool is AccessControl, Ownable {
    ...
    function collatDollarBalance() public view returns (uint256) {
        if (collateralPricePaused == true) {
            ...
            ).mul(10 ** missing_decimals).mul(pausedPrice).div(PRICE_PRECISION);
        } else {
            // Use
    ...
    function mint1t1EUSD(uint256 collateral_amount, uint256 EUSD_out_min)
        external
        notMintPaused
    {
        ...
        require(
            pid.global_collateral_ratio() >= COLLATERAL_RATIO_MAX,
        ...
        eusd_amount_d18 =
            (eusd_amount_d18.mul(uint256(1e6).sub(minting_fee))).div(1e6); //remove precision at the end
        ...
        notMintPaused
    {
        ...
        uint256 collateral_amount_d18 =
            collateral_amount * (10 ** missing_decimals);
        PoolLibrary.MintFF_Params memory input_params = PoolLibrary
            .MintFF_Params(
            share_price,
            getCollateralPrice(),
            share_amount,
        ...
        notRedeemPaused
    {
        ...
        (uint256 collateral_needed) = PoolLibrary.calcRedeem1t1EUSD(
            getCollateralPrice(), eusd_amount_precision
        );

        collateral_needed =
            (collateral_needed.mul(uint256(1e6).sub(redemption_fee))).div(1e6);
        require(
            collateral_needed
        ...
        require(
            COLLATERAL_out_min <= collateral_needed, "Slippage limit reached"
        );
        ...
        notRedeemPaused
    {
        ...
        uint256 share_amount =
            share_dollar_value_d18.mul(PRICE_PRECISION).div(share_price);

        // Need to adjust for decimals of collateral
        uint256 eusd_amount_precision =
            eusd_amount_post_fee.div(10 ** missing_decimals);
        ...
        redeemShareBalances[msg.sender] =
            redeemShareBalances[msg.sender].add(share_amount);
    ...
    function collectRedemption() external {
        ...
        if (redeemCollateralBalances[msg.sender] > 0) {
            ...
            unclaimedPoolCollateral =
                unclaimedPoolCollateral.sub(CollateralAmount);
        ...
        if (sendCollateral) {
            TransferHelper.safeTransfer(
                address(collateral_token), msg.sender, CollateralAmount
            );
        }
    }
        ...
        external
    {
        ...
        uint256 share_paid_back = amount_to_recollat.mul(
            uint256(1e6).add(bonus_rate).sub(recollat_fee)
        ).div(share_price);
    ...
    function buyBackShare(uint256 Share_amount, uint256 COLLATERAL_out_min)
        external
    {
        require(buyBackPaused == false, "Buyback is paused");
        uint256 share_price = priceOracle.getShareUSDPrice();

        PoolLibrary.BuybackShare_Params memory input_params = PoolLibrary
            .BuybackShare_Params(
        ...
        (uint256 collateral_equivalent_d18) = (
            PoolLibrary.calcBuyBackShare(input_params)
        ).mul(uint256(1e6).sub(buyback_fee)).div(1e6);
        uint256 collateral_precision =
            collateral_equivalent_d18.div(10 ** missing_decimals);

        require(
            COLLATERAL_out_min <= collateral_precision, "Slippage limit reached"
        );
        // Give the sender their desired collateral and burn the Share
        share.pool_burn_from(msg.sender, Share_amount);
        TransferHelper.safeTransfer(
            address(collateral_token), msg.sender, collateral_precision
        );
    }

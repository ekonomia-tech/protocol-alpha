...
library PoolLibrary {
    ...
    function calcMint1t1EUSD(uint256 col_price, uint256 collateral_amount_d18) public pure returns (uint256) {
    ...
    function calcMintFractionalEUSD(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        uint calculated_share_dollar_value_d18 = 
                    (c_dollar_value_d18.mul(1e6).div(params.col_ratio))
                    .sub(c_dollar_value_d18);

        uint calculated_share_needed = calculated_share_dollar_value_d18.mul(1e6).div(params.share_price_usd);

        return (
            c_dollar_value_d18.add(calculated_share_dollar_value_d18),
            calculated_share_needed
        );
    }

    function calcRedeem1t1EUSD(uint256 col_price_usd, uint256 EUSD_amount) public pure returns (uint256) {
    ...
    function calcBuyBackShare(BuybackShare_Params memory params) internal pure returns (uint256) {
        ...
        require(share_dollar_value_d18 <= params.excess_collateral_dollar_value_d18, "You are trying to buy back more than the excess!");

        // Get the equivalent amount of collateral based on the market value of Share provided 
        uint256 collateral_equivalent_d18 = share_dollar_value_d18.mul(1e6).div(params.col_price_usd);
    ...
    function calcRecollateralizeEUSDInner(
        ...
        uint256 recollat_possible = (global_collateral_ratio.mul(eusd_total_supply).sub(eusd_total_supply.mul(effective_collateral_ratio))).div(1e6);

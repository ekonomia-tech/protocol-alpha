...
import {IBondAuctioneer} from "@bondprotocol/interfaces/IBondAuctioneer.sol";
...
abstract contract BondBaseTeller is IBondTeller, ReentrancyGuard {
    ...
    constructor(address tonTimelock_, address protocol_, IBondAggregator aggregator_) {
        TONTimelock = tonTimelock_;
        _protocol = protocol_;
    ...
    /// @inheritdoc IBondTeller
    function setProtocolFee(uint48 fee_) external override onlyTONTimelock {
        if (fee_ > 5e3) revert Teller_InvalidParams();
        protocolFee = fee_;
    }

    /// @inheritdoc IBondTeller
    function setCreateFeeDiscount(uint48 discount_) external override onlyTONTimelock {
    ...
    function purchase(
        ...
        uint256 toProtocol =
            amount_.mulDiv(protocolFee + referrerFees[referrer_], FEE_DECIMALS) - toReferrer;
    ...
    function _handleTransfers(uint256 id_, uint256 amount_, uint256 payout_, uint256 feePaid_)
        internal
    {
        // Get info from auctioneer
        (address owner, address callbackAddr, ERC20 payoutToken, ERC20 quoteToken,,) =

...
import {IBondSDA, IBondAuctioneer} from "@bondprotocol/interfaces/IBondSDA.sol";
import {IBondTeller} from "@bondprotocol/interfaces/IBondTeller.sol";
import {IBondCallback} from "@bondprotocol/interfaces/IBondCallback.sol";
import {IBondAggregator} from "@bondprotocol/interfaces/IBondAggregator.sol";

import {TransferHelper} from "@bondprotocol/lib/TransferHelper.sol";
import {FullMath} from "@bondprotocol/lib/FullMath.sol";

/// @notice Forked from Bond Protocol - https://github.com/Bond-Protocol/bond-contracts
...
abstract contract BondBaseSDA is IBondSDA {
    ...
    // Implementation varibales
    address public TONTimelock;

    modifier onlyTONTimelock() {
        if (msg.sender != TONTimelock) {
            revert Auctioneer_NotAuthorized();
        }
        _;
    }

    constructor(address tonTimelock_, IBondTeller teller_, IBondAggregator aggregator_) {
    ...
    function _createMarket(MarketParams memory params_) internal returns (uint256) {
        ...
        uint32 debtDecayInterval;
        {
            ...
            debtDecayInterval =
                minDebtDecayInterval > userDebtDecay ? minDebtDecayInterval : userDebtDecay;
        ...
        uint256 maxPayout;
        {
            ...
            maxPayout =
                capacity.mulDiv(uint256(params_.depositInterval), uint256(secondsToConclusion));
        }
        ...
        uint256 maxDebt = targetDebt
            + targetDebt.mulDiv(
                uint256(params_.debtBuffer > minDebtBuffer_ ? params_.debtBuffer : minDebtBuffer_), 1e5
            );
    ...
    function setIntervals(uint256 id_, uint32[3] calldata intervals_) external override {
        ...
        meta.tuneIntervalCapacity = market.capacity.mulDiv(
            uint256(intervals_[0]), uint256(terms[id_].conclusion) - block.timestamp
    ...
    /// @inheritdoc IBondAuctioneer
    function setDefaults(uint32[6] memory defaults_) external override onlyTONTimelock {
    ...
    /// @inheritdoc IBondAuctioneer
    function setAllowNewMarkets(bool status_) external override onlyTONTimelock {
        // Restricted to authorized addresses
    ...
    /// @inheritdoc IBondAuctioneer
    function purchaseBond(uint256 id_, uint256 amount_, uint256 minAmountOut_)
        external
        override
    ...
    function _decayAndGetPrice(uint256 id_, uint256 amount_, uint48 time_)
        internal
        returns (uint256 marketPrice_, uint256 payout_)
    {
        BondMarket memory market = markets[id_];
        ...
        markets[id_].totalDebt =
            decayedDebt.mulDiv(debtDecayInterval, decayOffset + lastDecayIncrement) + payout_ + 1; // add 1 to satisfy price inequality
    }
    ...
    function _tune(uint256 id_, uint48 time_, uint256 price_) internal {
        ...
        uint256 capacity =
            market.capacityInQuote ? market.capacity.mulDiv(market.scale, price_) : market.capacity;
        ...
        uint256 timeNeutralCapacity = initialCapacity.mulDiv(
            uint256(meta.length) - timeRemaining, uint256(meta.length)
        ) + capacity;
        ...
        if (
            ...
            uint256 targetDebt =
                timeNeutralCapacity.mulDiv(uint256(meta.debtDecayInterval), uint256(meta.length));
    ...
    function _controlDecay(uint256 id_)
        internal
        view
        returns (uint256 decay, uint48 secondsSince, bool active)
    {
    ...
    /// @inheritdoc IBondAuctioneer
    function payoutFor(uint256 amount_, uint256 id_, address referrer_)
        public
        view
        override
        returns (uint256)
    ...
    function maxAmountAccepted(uint256 id_, address referrer_) external view returns (uint256) {
        ...
        uint256 quoteCapacity =
            market.capacityInQuote ? market.capacity : market.capacity.mulDiv(price, market.scale);

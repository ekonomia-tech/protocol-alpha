...
contract BondAggregator is IBondAggregator {
    ...
    error Aggregator_NotAuthorized();
    ...
    modifier onlyTONTimelock() {
        if (msg.sender != TONTimelock) {
            revert Aggregator_NotAuthorized();
        }
        _;
    }

    constructor(address tonTimelock_) {
        TONTimelock = tonTimelock_;
    }

    /// @inheritdoc IBondAggregator
    function registerAuctioneer(IBondAuctioneer auctioneer_) external onlyTONTimelock {

...
contract LiquidityGauge is ReentrancyGuard, IGauge {
    ...
    constructor(address _lp_addr, address _minter, address _ownership) {
        ...
        token = IToken(_token_addr);
        controller = IGaugeController(minter.controller());
        voting_escrow = IVotingEscrow(controller.voting_escrow());
        period_timestamp[0] = block.timestamp;
        inflation_rate = token.rate();
        future_epoch_time = token.future_epoch_time_write();
    ...
    function _update_liquidity_limit(address _addr, uint256 _l, uint256 _L) internal {
        ...
        uint256 _voting_balance = voting_escrow.balanceOf(_addr, block.timestamp);
        ...
        if ((_voting_total > 0) && (block.timestamp > period_timestamp[0] + BOOST_WARMUP)) {
            _lim += (_L * _voting_balance * (100 - TOKENLESS_PRODUCTION)) / _voting_total / 100;
        }
        ...
        emit UpdateLiquidityLimit(
            _addr, _l, _L, _lim, _working_supply, _voting_balance, _voting_total
            );
    }
    ...
    function _checkpoint(address _addr) internal {
        ...
        if (block.timestamp > _st.period_time) {
            ...
            uint256 _week_time;
            unchecked {
                _week_time = min(((_st.period_time + WEEK) / WEEK) * WEEK, block.timestamp);
            }

            for (uint256 i; i < 500;) {
                uint256 _dt = _week_time - _prev_week_time;
                uint256 _w =
                    controller.gauge_relative_weight(address(this), (_prev_week_time / WEEK) * WEEK);
                ...
                if (_working_supply > 0) {
                    if (
                        ...
                        _st.integrate_inv_supply += (
                            _st.rate * _w * (_st.prev_future_epoch - _prev_week_time)
                        ) / _working_supply;
                        _st.rate = _st.new_rate;
                        _st.integrate_inv_supply +=
                            (_st.rate * _w * (_week_time - _st.prev_future_epoch)) / _working_supply;
                    } else {
                        _st.integrate_inv_supply += (_st.rate * _w * _dt) / _working_supply;
                    }
        ...
        integrate_fraction[_addr] += (
            _working_balance * (_st.integrate_inv_supply - integrate_inv_supply_of[_addr])
        ) / 10 ** 18;
    ...
    function claimable_tokens(address _addr) external returns (uint256) {
        _checkpoint(_addr);
        return (integrate_fraction[_addr] - minter.minted(_addr, address(this)));
    }
    ...
    function kick(address _addr) external {
        ...
        uint256 _t_ve =
            voting_escrow.user_point_history__ts(_addr, voting_escrow.get_user_point_epoch(_addr));
        uint256 _balance = balanceOf[_addr];

        require(
            voting_escrow.balanceOf(_addr, block.timestamp) == 0 || _t_ve > _t_last,

...
contract TONGovernorBravoDelegate is GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    uint256 public constant MIN_PROPOSAL_THRESHOLD = 1000e18; // 1,000 TON

    /// @notice The maximum setable proposal threshold
    uint256 public constant MAX_PROPOSAL_THRESHOLD = 1000e18; // NOTE - originally set to 100,000 COMP but for us we are setting it to 1,000 TON for tests.
    ...
    uint256 public constant quorumVotes = 10000e18; // NOTE - originally set to 400,000 (4% of COMP) but for us we are setting it to 10,000 for tests.
    ...
    function propose(
        ...
        require(
            ton.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold
    ...
    function cancel(uint256 proposalId) external {
        ...
        if (msg.sender != proposal.proposer) {
            ...
            if (isWhitelisted(proposal.proposer)) {
                require(
                    (
                        ton.getPriorVotes(proposal.proposer, sub256(block.number, 1))
                            < proposalThreshold
        ...
        external
    {
        ...
        emit VoteCast(
            signatory, proposalId, support, castVoteInternal(signatory, proposalId, support), ""
            );
    }
     ...
     */
    function castVoteInternal(address voter, uint256 proposalId, uint8 support)
        internal
        returns (uint96)
    {
        require(

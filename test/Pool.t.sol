...
contract PoolTest is BaseSetup {
    function setUp() public {
        _fundAndApproveUSDC(
            user1, address(pool_usdc), tenThousand_d6, tenThousand_d6
        );
    }
    ...
    function testMint1t1EUSD() public {
        ...
        assertEq(
            balanceAfterMint.usdc, balanceBeforeMint.usdc - collateralAmount
        );
    }
    ...
    function testMintFractionalEUSD() public {
        ...
        _fundAndApproveShare(
            user1, address(pool_usdc), shareAmount, shareAmount
        );

        (uint256 collRequired, uint256 shareRequired) =
            _calcFractionalParts(totalToMint);
        ...
        assertEq(
            balanceAfterMint.share, balanceBeforeMint.share - shareRequired
        );
    }
    ...
    function testCannotMintFractionalWhenPaused() public {
        ...
        pool_usdc.mintFractionalEUSD(
            PRICE_PRECISION, one_d18, oneHundred_d18 - ten_d18
        );
    }
    ...
    function testRedeem1t1EUSD() public {
        ...
        uint256 unclaimedPoolCollateralBefore =
            pool_usdc.unclaimedPoolCollateral();
        ...
        vm.stopPrank();
        assertEq(
            balanceAfterRedeem.eusd, balanceBeforeRedeem.eusd - redeemAmount
        );
        assertEq(pool_usdc.redeemCollateralBalances(user1), fifty_d6);
        assertEq(
            pool_usdc.unclaimedPoolCollateral(),
            unclaimedPoolCollateralBefore + fifty_d6
        );
    ...
    function testRedeemFractionalEUSD() public {
        ...
        Balance memory balanceBeforeRedeem =
            _mintFractionalEUSD(user1, oneHundred_d18, 1020000);

        uint256 amountToRedeem = balanceBeforeRedeem.eusd / 2;
        (uint256 collPart, uint256 sharePart) =
            _calculateRedeemingParts(amountToRedeem);
        ...
        assertEq(
            balanceAfterRedeem.eusd, balanceBeforeRedeem.eusd - amountToRedeem
        );
    ...
    function testCollectRedemption() public {
        ...
        assertEq(
            userBalanceAfterCollection.share,
            userBalanceBeforeCollection.share + shareInPool
        );
        assertEq(
            userBalanceAfterCollection.usdc,
            userBalanceBeforeCollection.usdc + collateralInPool
        );
    }
    ...
    function testRecollateralizeEUSD() public {
        _mintEUSD(user1, oneHundred_d6);
        _fundAndApproveUSDC(
            user1, address(pool_usdc), 1000 * (10 ** 6), 1000 * (10 ** 6)
        );
        ...
        uint256 expectedShare = collateral_amount_d18
            * (10 ** 6 + pool_usdc.bonus_rate() - pool_usdc.recollat_fee())
            / priceOracle.getShareUSDPrice();
        ...
        assertEq(
            balanceAfterRecollat.usdc,
            balanceBeforeRecollat.usdc - collateral_amount
        );
        assertEq(
            usdc.balanceOf(address(pool_usdc)),
            poolCollateralBefore + collateral_amount
        );
    }
    ...
    function testBuyBackShares() public {
        ...
        _fundAndApproveShare(
            user1, address(pool_usdc), oneHundred_d18, oneHundred_d18
        );
        ...
        uint256 expectedCollateral = shareAmount * sharePrice / one_d18
            * (10 ** 6 - pool_usdc.buyback_fee()) / 10 ** 6;
        ...
        assertEq(
            balanceAfterBuyBack.usdc,
            balanceBeforeBuyBack.usdc + expectedCollateral
        );
        assertEq(
            balanceAfterBuyBack.share, balanceBeforeBuyBack.share - shareAmount
        );
    }
    ...
    function testCannotBuyBackShareSlippage() public {
        ...
        uint256 minOut =
            shareAmount * sharePrice / (10 ** 6) * 99 / 100 / (missing_decimals);
    ...
    function _calcFractionalParts(uint256 _totalEUSDOut)
        public
        returns (uint256 collRequired, uint256 shareRequired)
    {
        ...
        uint256 collDollarPortion =
            (totalDollarValue * gcr) / (missing_decimals);
        uint256 shareDollarPortion =
            (totalDollarValue * (PRICE_PRECISION - gcr)) / PRICE_PRECISION;
    ...
    function _calculateRedeemingParts(uint256 _redeemAmount)
        private
        returns (uint256 collateralAmount, uint256 shareAmount)
    {
        uint256 gcr = pid.global_collateral_ratio();
        uint256 sharePrice = priceOracle.getShareUSDPrice();
        uint256 usdcPrice = priceOracle.getUSDCUSDPrice();
    ...
    function _mintEUSD(address _to, uint256 _amountToMint)
        private
        returns (Balance memory)
    {
        _fundAndApproveUSDC(
            _to, address(pool_usdc), _amountToMint * 2, _amountToMint * 2
        );
        ...
        returns (Balance memory)
    {
        ...
        (uint256 collRequired, uint256 shareRequired) =
            _calcFractionalParts(_amountToMint);

...
contract PHOTWAPOracleTest is BaseSetup {
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        phoTwapOracle = new PHOTWAPOracle(address(pho), metaPoolFactoryAddress, fraxBPPool, fraxBPLPToken, fraxAddress, USDC_ADDRESS, period, fraxBPPhoMetapoolAddress, PRICE_THRESHOLD); // deploy PHOTWAPOracle
    ...
    //     assertEq(expectedPriceCumulativeLast0, fraxBPPhoMetapool.firstBalances(0));
    //     assertEq(expectedPriceCumulativeLast1, fraxBPPhoMetapool.firstBalances(1));
    //     assertEq(expectedBlockTimeStamp, fraxBPPhoMetapool.blockTimestampLast());
    //     assertEq(expectedTWAP0, fraxBPPhoMetapool.twap(0));
    //     assertEq(expectedTWAP1, fraxBPPhoMetapool.twap(1));
    // }
    ...
    //     vm.startPrank(owner);
    //     phoTwapOracle.getPrice();
    //     vm.warp(phoTwapOracle.blockTimestampLast() + period + 1);
    ...
    // // This helper does not care if price is above PRICE_THRESHOLD. It shouldn't be though, as there are separate tests checking for that requirement.
    ...
    //     uint256[2] memory lastTwap = phoTwapOracle.twap();
    //     uint256[2] memory expectedTwap;
    //     uint256[2] memory priceCumulativeLast = phoTwapOracle.priceCumulativeLast();

...
//     address public rewardToken = 0x6DEA81C8171D0bA574754EF6F8b412F2Ed88c54D; // LQTY
...
//         lusd.transfer(address(liquityDepositModule.liquityModuleAMO()), TEN_THOUSAND_D18);
//         vm.stopPrank();
...
//         IModuleAMO moduleRewardPool = _module.liquityModuleAMO();
//         LiquityModuleAMO amo = LiquityModuleAMO(address(_module.liquityModuleAMO()));
...
//         before.userStakedAmount = moduleRewardPool.stakedAmount(user1);
//         before.totalPHOSupply = pho.totalSupply();

//         // Check stability pool
//         before.liquityPoolDeposits =
//             amo.stabilityPool().getCompoundedLUSDDeposit(address(moduleRewardPool));
//         before.liquityPoolDepositorLQTYGain =
//             amo.stabilityPool().getDepositorLQTYGain(address(moduleRewardPool));
...
//         aft.userStakedAmount = moduleRewardPool.stakedAmount(user1);
//         aft.totalPHOSupply = pho.totalSupply();

//         // Check stability pool
//         aft.liquityPoolDeposits =
//             amo.stabilityPool().getCompoundedLUSDDeposit(address(moduleRewardPool));
...
//         LiquityModuleAMO moduleRewardPool = LiquityModuleAMO(address(_module.liquityModuleAMO()));
...
//         // Get reward
//         vm.prank(owner);
//         uint256 rewardsLiquity = moduleRewardPool.getRewardLiquity();

//         // User gets the reward
//         vm.warp(block.timestamp + 1 days);

//         vm.prank(user1);
//         moduleRewardPool.getReward(user1);

//         uint256 finalUserRewardsBalance = IERC20(moduleRewardPool.rewardToken()).balanceOf(user1);
...
//     ) public {
//         // Convert expected issue amount based on stablecoin decimals
//         // address moduleRewardPool = _module.liquityModuleAMO(); - not in use
